...
function directive(node, entering) {
    switch (node.directive)
    {
        ...
        case 'push':
            this.stylesStack.push(Object.assign({}, this.styles));
            break;

        case 'pop':
            this.styles = this.stylesStack.pop() ?? {};
            break;
    }

    /*
    let attrs = this.attrs(node);
    this.tag("directive", attrs);
    this.out(node.directive);
    this.tag("/directive");
    */
}
...
function block_quote(node, entering) {
    var attrs = this.attrs(node);
    if (entering) {
        this.stylesStack.push(this.styles);
...
function attrs(node) {
    ...
    {
        let styles = "";
        for (let k of Object.keys(this.styles))
        {
            styles += `${k}:${this.styles[k]};`
        }
        if (styles != "")
            att.push([ "style", styles ]);
    }
    return att;
}
