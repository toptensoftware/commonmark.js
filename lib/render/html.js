...
function if_branch(node, entering) {
    if (entering)
    {
        this.cr();
        let attrs = [ [ "id", `cond_${node.parent.id}_${node.branch}`] ];
        if (!node.branch)
...
function section(node, entering) {
    this.cr();
    if (entering)
    {
        this.tag("section", this.attrs(node));
    }
    else
    {
        this.tag("/section");
...
HtmlRenderer.prototype.if_branch = if_branch;
HtmlRenderer.prototype.section = section;
